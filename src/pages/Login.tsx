...
const LoginSignupScreens: React.FC<AuthPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      onLoginSuccess(user.name); // <--- A MÁGICA ACONTECE AQUI
    } else {
  ...
  const handleSignupSubmit = (e: React.FormEvent) => {
    ...
    // Simula o cadastro adicionando o usuário à nossa lista mockada
    ...
    onLoginSuccess(newUser.name); // <--- LOGA O USUÁRIO AUTOMATICAMENTE
  };
                ...
                <div>
                  <label className="flex items-start cursor-pointer">
                    ...
                    <span className="ml-2 text-sm" style={{ color: '#666666' }}>
                      Aceito os <button className="font-medium hover:opacity-80 transition" style={{ color: '#2E7D32' }}>termos de uso</button> e a <button className="font-medium hover:opacity-80 transition" style={{ color: '#2E7D32' }}>política de privacidade</button>
                    </span>
                  </label>
                </div>
